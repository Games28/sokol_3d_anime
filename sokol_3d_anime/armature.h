...
struct Armature {
	...
	std::vector<cmn::mat4> calculateAnimationPose(std::vector<cmn::mat4> mat_in) const {
		...
		for (int i = 0; i < bones.size(); i++) {
			mat_local[i] = cmn::mat4::mul( mat_in[i], bones[i].mat_local);
		}
		...
		for (int i = 1; i < bones.size(); i++) {
			...
			mat_model[i] = cmn::mat4::mul( mat_local[i], mat_model[p]);
		}

		std::vector<cmn::mat4> mat_out(bones.size());
		for (int i = 0; i < bones.size(); i++) {
			mat_out[i] = cmn::mat4::mul( mat_model[i], bones[i].mat_inv_model);
		}
